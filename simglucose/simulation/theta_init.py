...
class ThetaInit:
    ...
    def calculate_theta(self):
        self.g_sig = np.array(self.g_sig)
        ...
        IOB_max = self._calc_IOBmax(IOBbasal, IOB_TDI, dg_sig, d2g_sig)
        IA = IOB_max + basal

        IA = IA[self.d:]

        TE = te.te_compute(self.g_sig, IA, k=1, embedding=1, safetyCheck=True, GPU=False)
    ...
    def _calc_IOBmax(self, IOBbasal, IOB_TDI, dg_sig, d2g_sig):
        g_sig = self.g_sig[2:]
        dg_sig = dg_sig[1:]
        ...
        def calc(g, dg, ddg):
            ...
            if 150 <= g < 175 and dg > 0.25 and ddg > 0.035:
                return max(IOB_TDI, 2.5 * IOBbasal)

            if 175 <= g < 200 and dg > 0.35 and ddg > 0.035:
